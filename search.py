...
def choose_best_move(board, depth):
    ...
    for move in legal_moves:
        ...
        quiescent_eval = search_quiescent(new_board, depth-1, float('-inf'), float('inf'), False)
...
def search_quiescent(board, depth, alpha, beta, maximizing_player):
    if depth == 0 or board.is_game_over():
        return evaluate(board)
    ...
    if maximizing_player:
        ...
        for move in legal_captures:
            new_board = board.copy()
            new_board.push(move)
            eval_score = search_quiescent(new_board, depth - 1, alpha, beta, False)
            max_eval = max(max_eval, eval_score)
            alpha = max(alpha, eval_score)
            if beta <= alpha:
                break
        ...
        for move in legal_captures:
            new_board = board.copy()
            new_board.push(move)
            eval_score = search_quiescent(new_board, depth - 1, alpha, beta, True)
            min_eval = min(min_eval, eval_score)
            beta = min(beta, eval_score)
            if beta <= alpha:
                break
